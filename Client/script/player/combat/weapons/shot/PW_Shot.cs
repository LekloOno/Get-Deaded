...
public abstract partial class PW_Shot : WeaponComponent, GC_IHitDealer
{
    ...
    {
        get => _entitiesHitMask;
        protected set
        {
            _entitiesHitMask = value;
            UpdateHitMask();
        }
    }
    protected CONFE_HitMasks _entitiesHitMask = CONFE_HitMasks.Enemies;
    [Export] public bool ThroughWalls
    {
        get => _throughWalls;
        protected set
        {
    ...
    protected bool _throughWalls = false;

    private void UpdateHitMask()
    {
        _hitMask = _entitiesHitMask.ToCollisionMask();
        if (!_throughWalls)
            _hitMask |= CONF_Collision.Layers.Environment;
    }

    protected uint _hitMask;
