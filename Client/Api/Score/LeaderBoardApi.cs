...
public class LeaderboardApi : ApiClient
{
    public async Task<ApiResult<List<LeaderboardRowDto>>> GetWindowAsync(
        string mapKey, string modeKey, int difficulty, int centerRank, int take, CancellationToken ct = default) =>
        await SendAsync<List<LeaderboardRowDto>>(HttpMethod.Get,
            $"api/leaderboard?mapKey={mapKey}&modeKey={modeKey}&difficulty={difficulty}&centerRank={centerRank}&take={take}", ct: ct);
